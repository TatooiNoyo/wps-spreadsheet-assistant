...
public abstract class AbstractExcelService<S extends IService4Excel<T>, T, EI, EO> implements IExcelService {
     ...
     */
    protected ExcelFileDescriptor buildExcelFileDescriptor(String filename) {
        return new ExcelFileDescriptor(
                filename + ".xlsx",
    ...
    @Override
    public void downloadTemplate(ExcelOutput output) {
        List<EI> importElements = generateTemplateData();
        String fileName = getFilename();
        try {
            output.applySuggestedHeaders(buildExcelFileDescriptor(fileName));
    ...
    @Override
    public ImportResult importExcel(ExcelInput file) {
        try {
     ...
     */
    protected void prepareCheckFile(ExcelInput file) {
        if (null == file) {
            throw new RuntimeException("文件不能为空");
        }
        String extName = Optional.of(file).map(ExcelInput::getOriginalFilename)
    ...
    @Override
    public void exportExcel(ExcelOutput output, @Nonnull List<String> ids) {
     ...
     *
     * @param output  Excel输出对象
     * @param batchSize 每次查询的数据量，建议500-1000条
     */
    @Override
    public void exportLargeData(ExcelOutput output, int batchSize) {
        if (batchSize <= 0) {
            batchSize = 500;
        }
