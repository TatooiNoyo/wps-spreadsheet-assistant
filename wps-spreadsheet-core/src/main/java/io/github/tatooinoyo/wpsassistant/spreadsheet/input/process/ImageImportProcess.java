...
import io.github.tatooinoyo.wpsassistant.spreadsheet.IGetImage;
...
public class ImageImportProcess<T, EI> implements ImportProcess<T, EI> {
    ...
    public T process(RowWrapper<EI> row, T po, ImportContext context) {
        ...
        ISetImage setImageObj;
        IGetImage getImageObj;
        if (po instanceof ISetImage) {
            setImageObj = (ISetImage) po;
        } else {
            log.error("图片处理的实体必须是 ISetImage 的超集!");
            context.abort();
            return null;
        }
        if (row.getData() instanceof IGetImage) {
            getImageObj = (IGetImage) row.getData();
        } else {
            log.error("图片处理流程的Excel记录必须是 IGetImage 的超集!");
            context.abort();
            return null;
        }
        // 获取或收集图片关系字典
